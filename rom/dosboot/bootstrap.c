...
static BOOL GetBootNodeDeviceUnit(struct BootNode *bn, BPTR *device, IPTR *unit, ULONG *bootblocks)
{
    ...
    if (bn == NULL)
    {
        D(bug("[DOSBoot:bootstrap] %s: No BootNode\n", __func__));
        return FALSE;
    }
    ...
    if (dn == NULL)
    {
        D(bug("[DOSBoot:bootstrap] %s: No DeviceNode\n", __func__));
        return FALSE;
    }
    ...
    if (fssm == NULL)
    {
        D(bug("[DOSBoot:bootstrap] %s: No FileSysStartupMsg\n", __func__));
        return FALSE;
    }
...
BOOL dosboot_DevicePresent(struct IORequest*bootDevIO, BPTR bootDevName, ULONG bootDevUnit)
{
    ...
    D(bug("[DOSBoot:bootstrap] %s: Checking for device/media presence on %s.%d\n", __func__, device, bootDevUnit));
    if (!OpenDevice(device, bootDevUnit, bootDevIO, 0))
    {
        bootDevIO->io_Command = TD_CHANGESTATE;
        ((struct IOStdReq *)bootDevIO)->io_Actual = 1;
        DoIO(bootDevIO);
        if (!bootDevIO->io_Error && ((struct IOStdReq *)bootDevIO)->io_Actual == 0)
        {
            D(bug("[DOSBoot:bootstrap] %s: Present\n", __func__));
            return TRUE;
        }
        CloseDevice(bootDevIO);
    }
    return FALSE;
}
...
static BOOL dosboot_BootBlock(struct IOStdReq *io, struct BootNode *bn, struct ExpansionBase *ExpansionBase, ULONG bootblock_size)
{
    ...
    if (buffer != NULL)
    {
        ...
        D(bug("[DOSBoot:bootstrap] %s: bootblock read (%d bytes)\n", __func__, bootblock_size));
        DoIO((struct IORequest*)io);

        if (io->io_Error == 0)
        {
            D(bug("[DOSBoot:bootstrap] %s: bootblock read to %p ok\n", __func__, buffer));
            ...
            {
                D(bug("[DOSBoot:bootstrap] %s: Not a valid bootblock\n", __func__));
            }
        } else {
            bug("[DOSBoot:bootstrap] io_Error %d\n", io->io_Error);
        }
    ...
    if (init != NULL)
    {
       D(bug("[DOSBoot:bootstrap] %s: calling bootblock loaded code at %p\n", __func__, init));
...
LONG dosboot_BootStrap(LIBBASETYPEPTR LIBBASE)
{
    ...
    for (i = 0; i < nodes; i++)
    {
        ...
        {
            struct IOStdReq *io;
            D(bug("[DOSBoot:bootstrap] %s: Device IO MsgPort @ 0x%p\n", __func__,msgport));
            ...
            if ((io = CreateIORequest(msgport, sizeof(struct IOStdReq))))
            {
                ...
                BPTR device;
                IPTR unit;
                BOOL BootNodeDeviceUnit;

                D(bug("[DOSBoot:bootstrap] %s: Device IORequest @ 0x%p\n", __func__, io));
