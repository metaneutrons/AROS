...
void core_Switch(void)
{
    ...
    if (task->tc_State != TS_RUN && task->tc_Node.ln_Succ != NULL)
        Remove(&task->tc_Node);
...
struct Task *core_Dispatch(void)
{
    ...
    /*
     * Null stale node pointers immediately after REMHEAD.
     * core_Switch() checks (tc_State != TS_RUN) before calling Remove().
     * If the task calls Wait() (setting tc_State = TS_WAIT) before the
     * next cpu_Switch, core_Switch would Remove() with stale pointers,
     * corrupting whichever list those pointers reference.
     * With NULL pointers, our cpu_Switch detects the unlinked state
     * and enqueues the task directly without calling core_Switch's Remove.
     */
    if (task)
    {
        task->tc_Node.ln_Succ = NULL;
        task->tc_Node.ln_Pred = NULL;
    }
    if (!task)
    {
