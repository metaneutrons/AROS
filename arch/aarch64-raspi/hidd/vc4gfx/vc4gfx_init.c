...
static int FNAME_SUPPORT(Init)(LIBBASETYPEPTR LIBBASE)
{
    ...
    bug("[VideoCoreGfx] %s: Initialising...\n", __PRETTY_FUNCTION__);

    KernelBase = OpenResource("kernel.resource");
    __arm_periiobase = KrnGetSystemAttr(KATTR_PeripheralBase);

    bug("[VideoCoreGfx] %s: periiobase=0x%p\n", __PRETTY_FUNCTION__, __arm_periiobase);
    ...
    {
        bug("[VideoCoreGfx] %s: Failed to open mbox.resource\n", __PRETTY_FUNCTION__);
        goto failure;
    }
    ...
    {
        if (FNAME_SUPPORT(InitMem)((void*)(IPTR)AROS_LE2LONG(xsd->vcsd_MBoxMessage[5]), AROS_LE2LONG(xsd->vcsd_MBoxMessage[6]), LIBBASE))
        {
